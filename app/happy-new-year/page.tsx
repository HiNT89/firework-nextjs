...
// Component Lì xì
const LuckyMoney: React.FC<{ link: string }> = ({ link }) => {
  const [screenHeight, setScreenHeight] = useState(0);
...
const HappyNewYear = () => {
  ...
  const wishes = useMemo(() => {
    ...
    return wishList
      ...
      .slice(0, 4)
      .map((w) =>
        w.replace("anh", userName || "bạn").replace("chị", userName || "bạn"),
      );
  }, [gender, userName]);
  ...
  useEffect(() => {
    if (!TIMING_CONFIG.fireworkCycleEnabled) return;
  ...
  return (
    <main className="bg-black h-screen w-screen text-white relative overflow-hidden">
      ...
      <AnimatePresence>
        {showLuckyMoney && <LuckyMoney link={luckyMoneyLink} />}
      </AnimatePresence>
    </main>
  );
};
